    ...
    return (
      <div className="shadow-card w-full lg:h-[360px] rounded-2xl bg-white flex flex-col justify-start items-center">
        ...
        <div className="px-5">
          ...
          <h1 className="text-[13px] font-semibold text-black text-center mb-1">
            {product?.price.toLocaleString("en-US")} MMK
          </h1>
        ...
        <div className="px-3 w-full flex justify-between items-center mt-auto mb-3">
          {Number(product.stock) > 0 ? (
            <h1 className="text-[11px]">In stock {product.stock}</h1>
          ) : (
            <h1 className="text-[11px] text-red-500">Sold out</h1>
          )}{" "}
          <Button size={"small"} variant="contained">
            <Link to={`/detail/${product.id}`}>Buy</Link>
          </Button>
        </div>
      </div>
    );
